...
def test_plain_human_commit_has_no_agent() -> None:
    human, agent, model = parse_authors("Nat", "fix the thing")
    assert (human, agent, model) == ("Nat", "", "")


def test_agent_trailer_is_split_into_vendor_and_model() -> None:
    body = "feat: add thing\n\nCo-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>"
    assert parse_authors("Nat", body) == ("Nat", "Claude", "Sonnet 5")


def test_the_other_trailer_spelling_is_accepted() -> None:
...
def test_human_co_author_is_not_recorded_as_an_agent() -> None:
...
def test_numstat_and_raw_lines_are_distinguishable() -> None:
...
def test_binary_files_count_as_zero_not_a_crash() -> None:
    assert _num("-") == 0
    assert _num("12") == 12
